...
db = app.LoadText("user",
	"{\"times\":1,\"mode\":\"light\",\"history\":[],\"favourite\":[],\"lastRead\":null,\"lastTime\":null}",
	datacard);
ldb = JSON.parse(db); //liquid database
//app.ShowPopup( ldb.times )
	...
	constructor (bol) {
		...
		this. retry = ()=> {
			if (!this.d) {
				...
				//app.ShowPopup( "retrying" )
			}
		}

		this.wait = ()=> {
			//app.ShowPopup( "waiting" );
			setTimeout(()=> {
				this.retry();
			}, 20000);
				...
				const netWork = setInterval(()=> {
					if (html) {
						...
						try {
							...
							//const a=JSON.stringify(adData);
							//app.Alert( a )

							//const index=publicData.indexOf(adData);
							//app.ShowPopup( index )
							//return true
							//app.Alert(typeof(html ))
							...
							app.GA("send", "screenview",
								{
									"appName": "HolyBible", "appVersion": ver, "screenName": "adView"
								})
...
class layout
{
	...
	constructor(bol) {
		...
		this.titleText = app.AddText(this.title, "HOLY BIBLE", -1, -
			1, "bold");
...
class readMode extends layout
{
	...
	addHLog = (item) =>
	{
		...
		ldb.history.includes(item) ? verify(): ldb["history"].unshift(
			item);
	...
	constructor(book, chapter, bol) {
		...
		this.favText = app.AddText(this.title, "[fa-star]", 0.16, -
			1, "FontAwesome")
		this.favText.SetTextSize(30)
		this.favText.SetMargins(NaN, 0.015, -0.15)

		this.favText.SetOnTouchUp(this.addFav)
		ldb.favourite.includes(this.hLog) ? this.favText.SetTextColor(
			"red"): this.favText.SetTextColor("blue")
		...
		this.text = app.AddText(this.scrlPad, "", 0.95, NaN,
			"multiline,left");
		...
		try
		{
			...
			// app.TextToSpeech( bibleO[book][chapter] )
		...
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "ReadMode"
			})
...
class favourite extends layout
{
	...
	openRead = (log) =>
	{
		//called when favoirite is touched
	...
	constructor(bol) {
		...
		this.list = app.AddList(this.scrlPad, ldb.favourite.join(),
			0.98, NaN, "Expand")
		...
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "favourite"
			})
...
class history extends layout
{
	...
	constructor(bol) {
		...
		this.list = app.AddList(this.scrlPad, ldb.history.join(),
			0.98, NaN, "Expand")
		this.list.SetTextSize(22);
      ...
      */
		...
		//app.Alert( arr .length)
		arr.forEach((value, index)=> {
			arr[index] = value.title;
		});
		//app.SetClipboardText( arr );
		//app.SetClipboardText( ldb.history )
		//app.ShowPopup(typeof( arr))
		//arr==ldb.history?app.ShowPopup( true ):app.Alert( "false" )


		const jl = JSON.stringify(arr);
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "history"
			})
...
class home extends layout
{
	...
	constructor() {
...

		{
			...
			this.text = app.AddText(this.scrlPad2, "", 0.91, NaN,
				"multiline,left");
...

		{
			...
			// this.newT.SetMargins(NaN,0.008);
			...
			//this.randV.SetBackColor("#f4f6f7");
			...
			//this.rcVc.SetOnTouchUp(randChap);
		...
		//  this.t = app.AddText(this.scrlPad, "\n\n", 0.1, 0.2,  "multiline")
...
class chapter extends layout
{
	...
	{
		openLays.push(new readMode(openLays[openLays.length - 1].name,
			chapter, true));
	...
	constructor(name,
		...
		this.list = app.AddList(this.scrlPad,
			this.listitem.join(),
			0.985,
			null,
			"Expand");
		this.list.SetTextSize(22);
		this.list.SetMargins(0.01,
			0.01,
			0.01,
			0.1)
...
{
	//displays old and nee testament
	...
	constructor(test) {
		...
		//Alert(null,this.listitem)

		this.list = app.AddList(this.scrlPad, listitem.join(), 0.985,
			null, "Expand");
		this.list.SetTextSize(22);
...
class Program
{
	...
	start = () =>
	{
		...
		// openLays[0].otVc.SetOnTouchMove( this.doNothing)

		openLays[0].ntVc.SetOnTouch(this.openNewT)
		//openLays[0].ntVc.SetOnTouchMove( this.doNothing)

		openLays[0].oldText.SetOnTouch(this.openOldT)
		// openLays[0].oldText.SetOnTouchMove( this.doNothing)

		openLays[0].newText.SetOnTouch(this.openNewT)
		// openLays[0].newText.SetOnTouchMove( this.doNothing)
	...
	drawListAction = (name) =>
	{
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "drawer"
			})
...
function randChap(typ) {
	...
	if (typ) {
		...
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "random verse"
			})
		...
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "random chapter"
			})
...
function dailyRead() {
	...
	if (m != ldb.lastTime) {
		...
		//openLays.push(new readMode(dl[0],dl[1],true));
		...
		app.GA("send", "screenview",
			{
				"appName": "HolyBible", "appVersion": ver, "screenName": "daily read"
			})
...

{
	...
	drawerListItems =
	"SHOP,DISABLE SCREEN TURN OFF,HOW TO USE,PRIVACY POLICY,CONTACT US,SHARE APP,HISTORY,FAVOURITE,OLD TESTAMENT,NEW TESTAMENT,HOME"

	dlist = app.AddList(vc, drawerListItems, 0.75, NaN, "Expand")
	dlist.SetOnTouch(program.drawListAction)
	//dlist.SetTextSize( 20 )
...
function proposeExit() {
	...
	app.GetDrawerState() == "Open" ? app.CloseDrawer("left"):
	false;
